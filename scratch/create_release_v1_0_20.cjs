...
const createRelease = () => {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({
      ...
      name: TAG,
      body: 'Release v1.0.20 addressing: \n1. Implemented database auto-migration to automatically generate and save parent series cards offline for downloaded cbz chapters to ensure no previous downloaded content is lost after upgrading.',
      draft: false,
      prerelease: false
    });
...
(async () => {
  try {
    console.log('Creating release v1.0.20 on GitHub...');
