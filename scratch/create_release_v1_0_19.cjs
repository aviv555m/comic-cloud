...
const createRelease = () => {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({
      ...
      name: TAG,
      body: 'Release v1.0.19 addressing: \n1. Added Download App option to navigation bar on the website (hidden on native views).\n2. Configured native update check system querying GitHub API once upon app open to alert users on update availability.',
      draft: false,
      prerelease: false
    });
...
(async () => {
  try {
    console.log('Creating release v1.0.19 on GitHub...');
