    ...
    if (Capacitor.isNativePlatform()) {
      const checkNativeUpdate = async () => {
        try {
          ...
          const currentTag = "v1.0.91"; // Hardcoded current native app version
        ...
        <div className="w-full max-w-md bg-card border border-violet-500/20 rounded-2xl p-6 shadow-2xl space-y-4">
          <div className="flex flex-col items-center text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              You are running version <span className="text-muted-foreground/80 font-mono font-bold">v1.0.91</span>. A mandatory update to <span className="text-violet-400 font-bold font-mono">{latestReleaseInfo.tag}</span> is required to continue.
            </p>
          </div>
